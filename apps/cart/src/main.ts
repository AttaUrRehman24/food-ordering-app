...
import {
  AppModule,
  bootstrapCartInfrastructure,
  shutdownCartInfrastructure,
} from './app.module';
...
async function bootstrap(): Promise<void> {
  ...
  app.connectMicroservice<MicroserviceOptions>({
    ...
    options: {
      package: 'foodordering.cart.v1',
      protoPath: join(process.cwd(), 'libs/proto/protos/cart.proto'),
  ...
  await bootstrapCartInfrastructure(app);
  await app.startAllMicroservices();
  await app.listen(HTTP_PORT);

  logJson(
    createStructuredLog(SERVICE_NAME, 'info', 'Cart Service ( Documentation §3.2) listening', null, {
      http_port: HTTP_PORT,
      grpc_port: GRPC_PORT,
    }),
  );
  ...
  const shutdown = async () => {
    await shutdownCartInfrastructure(app);
    await app.close();
    process.exit(0);
  };
