...
import {
  AppModule,
  bootstrapOrderInfrastructure,
  shutdownOrderInfrastructure,
} from './app.module';
...
async function bootstrap(): Promise<void> {
  ...
  app.connectMicroservice<MicroserviceOptions>({
    ...
    options: {
      package: 'foodordering.order.v1',
      protoPath: join(process.cwd(), 'libs/proto/protos/order.proto'),
  ...
  await bootstrapOrderInfrastructure(app);
  await app.startAllMicroservices();
  await app.listen(HTTP_PORT);

  logJson(
    createStructuredLog(SERVICE_NAME, 'info', 'Order Service ( Documentation §3.2) listening', null, {
      http_port: HTTP_PORT,
      grpc_port: GRPC_PORT,
    }),
  );
  ...
  const shutdown = async () => {
    await shutdownOrderInfrastructure(app);
    await app.close();
    process.exit(0);
  };
